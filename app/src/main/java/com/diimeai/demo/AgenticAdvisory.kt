...
object AgenticAdvisory {
    ...
    private val CAUSES: Map<String, SignalCause> = mapOf(
        ...
        "HOOKING_FRAMEWORK" to SignalCause(
            ...
            rootCause     = "A dynamic instrumentation framework (Frida server gadget, Xposed/LSPosed module, or Cydia Substrate hook) has been injected into this process's address space.",
            likelyOrigin  = "Attacker using Frida to hook authentication functions, bypass PIN/biometric checks, or extract session tokens from memory",
            riskAssessment = "CRITICAL: Attacker has full control over all function calls in this process. Authentication checks, encryption keys, and network responses can all be manipulated.",
            confidence    = "HIGH",
            socAction     = "SDK has terminated the process. Flag account for immediate security review. Invalidate all active sessions.",
        ),
        "PTRACE_ATTACHED" to SignalCause(
            signalType    = "PTRACE_ATTACHED",
            rootCause     = "A debugger process has attached to this application via the Linux ptrace system call, gaining the ability to read/write any memory, set breakpoints, and modify registers.",
            likelyOrigin  = "GDB or LLDB debugger via ADB, or malware using ptrace to inspect authentication flows and extract decrypted data",
            riskAssessment = "Attacker can read decrypted session tokens, PIN values, and cryptographic keys directly from process memory at any time.",
            confidence    = "HIGH",
            socAction     = "SDK has terminated the process. Consider this session fully compromised.",
        ),
        "NATIVE_LIB_TAMPER" to SignalCause(
            signalType    = "NATIVE_LIB_TAMPER",
            rootCause     = "Core native libraries mapped into the process have been patched or replaced at runtime. The in-memory content differs from the original on-disk binary.",
            likelyOrigin  = "LD_PRELOAD injection, Frida IL2CPP hook, or direct memory patching by a root-level tool",
            riskAssessment = "Security-critical native functions (certificate pinning, key derivation, attestation) may have been replaced with attacker-controlled code.",
            confidence    = "HIGH",
            socAction     = "SDK has terminated the process. Require device re-enrollment.",
        ),
        "APP_TAMPERING" to SignalCause(
            signalType    = "APP_TAMPERING",
            rootCause     = "The running APK's integrity differs from the code-signed original. On debug builds this fires because debuggable=true enables tampering-equivalent access.",
            likelyOrigin  = "On debug builds: expected, developer-only signal. On production builds: attacker has patched DEX bytecode or resources after signing.",
            riskAssessment = "On production: app code has been modified post-signing. Core security logic may be bypassed.",
            confidence    = "HIGH",
